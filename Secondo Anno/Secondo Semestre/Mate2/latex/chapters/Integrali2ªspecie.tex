...
Sia F un campo vettoriale conservativo e continuo in un dominio $D$ e $C$ una curva regolare orientata contenuta in $D$ vale la formula
\begin{definizione}{Teorema fondamentale del calcolo per gli integrali di linea}
    $$
        \int_{C} F \cdot t d\ell = U(P_{1}) - U(P_{0})
    $$
    \begin{itemize}
        \item $F$ : Rappresenta il \textbf{campo vettoriale} oggetto dell'integrazione, il quale deve essere \uline{continuo e rigorosamente conservativo} all'interno del dominio considerato
        \item $C$ : È la \textbf{curva regolare orientata} lungo la quale viene effettuato il calcolo integrale
        \item $t$ : È il \textbf{versore tangente} alla curva, diretto seguendo la specifica orientazione stabilita per $C$
        \item $d\ell$ : Costituisce \textbf{l'elemento differenziale} della curva
        \item $U$ : È la \textbf{funzione potenziale} associata al campo vettoriale.

        Affinchè il teorema sia valido, il campo $F$ deve derivare proprio dal gradiente di questa funzione scalare $(F=\nabla U)$
        \item $P_0$ e $P_1$ : Sono i \textbf{punti estremi della curva} $C$, dove $P_0$ corrisponde al punto di partenza e $P_1$ al punto di arrivo
    \end{itemize}
\end{definizione}
\subsection{A cosa serve?}
\subsection{Come usare l'enunciato}
\subsection{Dimostrazione}
\begin{enumerate}
    \item \textbf{Definizione degli estremi e della parametrizzazione}
        \begin{itemize}
            \item \textbf{Procedura}

            Si parte considerando una parametrizzazione regolare del cammino di integrazione $(x_{1}(t),\dots,x_{n}(t))$ che sia concorde con l'orientazione della curva.

            Il parametro varia tra un tempo $t_{0}$(associato al punto iniziale $P_0$) e un tempo $t_1$(associato al punto finale $P_1$)
